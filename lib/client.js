...
Client.prototype.trace = function(trace){
  ...
  Object.keys(handlers).forEach(function(pattern){
    var fn = handlers[pattern];
    var re = regexps[pattern] || (regexps[pattern] = utils.pattern(pattern));
    if (re.test(pattern)) fn(trace);
  });
};
